# Security Compliance Admin — 요구사항 명세서 (v0.2)
보안성 심사 대응용 어드민 시스템
...
- 설명: 발급/폐기, 사용 이력(최소 최종 사용시각/엔드포인트)
- 수용 기준:
...
<a id="fr-060"></a>
<details>
  <summary><strong>FR-060 운영 통계 대시보드</strong> — <em>MUST</em></summary>

- 설명: 고객사별 API Key 사용 건수, 시간대별 오류율 등을 조회할 수 있는 운영자 대시보드
- 수용 기준:
  - 고객사별 기간/엔드포인트별 API 호출 건수 집계
  - HTTP 상태코드 기준 성공/실패율, 시간대별 분포 차트 제공
  - OPERATOR/ADMIN 권한만 접근 가능
  - CSV 내보내기 지원
...
<a id="fr-070"></a>
<details>
  <summary><strong>FR-070 운영 로그 대시보드</strong> — <em>MUST</em></summary>

- 설명: DEVELOPER 전용, 고객사 데이터 다운로드 및 자신의 활동 로그를 조회하는 대시보드
- 수용 기준:
  - 사용자별 다운로드 요청/실행 이력 조회
  - 최근 실패/거부 내역 확인
  - 고객사별 데이터 접근 로그 조회
  - DEVELOPER/ADMIN 권한만 접근 가능
  - CSV 내보내기 지원
