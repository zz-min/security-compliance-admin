...
- 변경이력:
  ...
  - v0.2 (2025-09-08) UC-002,UC-004,UC-005,UC-006,UC-007 수정
...
- **예외**:
  ...
  - [E2] 세션 스토리지(redis) 장애로 세션 확인 불가 
	  ...
	  - 시스템 오류 로그 기록  
  - [E3] **관리자가 계정을 `INACTIVE`로 변경한 경우**  
    - 응답: `403 Forbidden` (`AUTH_ACCOUNT_INACTIVE`)  
    - 세션은 물리적으로 남아있지만, 모든 요청이 거부됨, 감사 로그 기록 (사유: 계정 비활성화 상태에서 요청 발생)  
- **대체 시나리오 (Alternative Flow)**:
  - 없음 (강제 로그아웃 기능 제거)
...
- **시나리오**:
  ...
  4. 페이징된 결과를 화면에 표시
  5. 사용자가 CSV 내보내기 요청 시 파일 생성후 다운로드
- **예외**:
  - [E1] 무권한 사용자 접근
	  - 응답: `403 Forbidden`
 ...
 - [E3] 조회 결과 없음
	  - 응답: `200 OK`
	  - 빈 리스트 반환 (에러 아님, 단 사용자에게 "결과 없음" 안내 메시지 표시),감사로그에 "조회 성공(0건)" 기록
- **대체 시나리오**:
  - 없음 (대용량 데이터 조회 및 다운로드 제거)
...
- **예외**:
  ...
  - [E2] 통계 데이터 수집 실패
	  ...
	  - 일시적 오류 메시지 표시
- **대체 시나리오**:
  - 없음 (실시간 모니터링 모드 제거, 조회버튼 눌렀을 때 기준으로만 조회)
